...
See >>[notebook](notebooks/Toy_dataset_1.ipynb)<< for the complete procedure.

This fit generates 44 candidate functions in total! The output files can be found [here](https://github.com/hftsoi/symbolfit/tree/main/docs/demo/notebooks/output_Toy_dataset_1) (feel free to download them and look at what a typical fit will produce).
...
{{ read_csv("notebooks/output_Toy_dataset_1/candidates_reduced.csv") }}
...
Now, lets take a look at one of the candidate functions, say candidate #43.
...
Here, the function has 164.796\*(...), this is the result of `scale_y_by = 'mean'` as the input y was scaled by its mean for when performing the fits (to avoid numerical overflow), so this factor is to unscale it after the fits are done so that the final function describes the original input dataset. 
Also, there is `(x0 - 12.5) * 0.00210526` appearing everywhere, this is the result of `input_rescale = True` as the input x was scaled to [0,1] when performing the fits and then unscaled afterward.
