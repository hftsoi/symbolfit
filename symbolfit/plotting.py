...
def plot_single_syst_single_func_1D(
    func_candidate, candidate_idx, x, bin_widths_1d, y, y_up, y_down, param_shifted, logy, logx
):
    """
    ...
    central = func_evaluate(func_candidate=func_candidate, x=x0, dim=1, param_shifted=None, sigma_pm=None)
    ...
    if param_shifted is not None:
        if func_candidate["Parameters: (best-fit, +1, -1)"][param_shifted][1] > 0:
            up = func_evaluate(func_candidate=func_candidate, x=x0, dim=1, param_shifted=param_shifted, sigma_pm="+")

            down = func_evaluate(func_candidate=func_candidate, x=x0, dim=1, param_shifted=param_shifted, sigma_pm="-")
    ...
    central_hist = func_evaluate(func_candidate=func_candidate, x=x, dim=1, param_shifted=None, sigma_pm=None)

    # Function evaluated with one of the parameters shifted up and down separately (with the same input x grid).
    if param_shifted is not None:
        if func_candidate["Parameters: (best-fit, +1, -1)"][param_shifted][1] > 0:
            up_hist = func_evaluate(
                func_candidate=func_candidate, x=x, dim=1, param_shifted=param_shifted, sigma_pm="+"
            )

            down_hist = func_evaluate(
                func_candidate=func_candidate, x=x, dim=1, param_shifted=param_shifted, sigma_pm="-"
            )
    ...
    if y_up is not None and y_down is not None:
        ...
        axes[0].scatter(x, y, marker="o", c="black", alpha=1, label="Data")

    # Plot the candidate function with all parameters in their best-fit values.
    axes[0].plot(x0, central, color="r", label="Best-fit", linewidth=linewidth, alpha=alpha)

    # Plot the candidate function with one of the parameters shifted up and down separately.
    if param_shifted is not None:
        if func_candidate["Parameters: (best-fit, +1, -1)"][param_shifted][1] > 0:
            axes[0].plot(
                x0, up, color="g", label=f"{param_shifted} Up", linewidth=linewidth, alpha=alpha, linestyle="dashed"
            )

            axes[0].plot(
                x0, down, color="b", label=f"{param_shifted} Down", linewidth=linewidth, alpha=alpha, linestyle="dashed"
            )
    ...
    for i in range(len(parameters)):
        ...
        if parameters[f"a{i + 1}"][1] == 0:
            if f"a{i + 1}" == param_shifted:
                parameters_string += r"$\bf{{{} = {}}}$".format(
                    f"a{i + 1}",
                    round_a_number(parameters[f"a{i + 1}"][0], 6),
                )

            else:
                parameters_string += r"${} = {}$".format(
                    f"a{i + 1}",
                    round_a_number(parameters[f"a{i + 1}"][0], 6),
                )
        else:
            if f"a{i + 1}" == param_shifted:
                parameters_string += r"$\bf{{{} = {}^{{+ {} ({}\%) }}_{{- {} ({}\%)}}}}$".format(
                    f"a{i + 1}",
                    round_a_number(parameters[f"a{i + 1}"][0], 6),
                    round_a_number(parameters[f"a{i + 1}"][1], 4),
                    round_a_number(
                        100 * np.abs(float(parameters[f"a{i + 1}"][1]) / float(parameters[f"a{i + 1}"][0])), 3
                    ),
                    round_a_number(np.abs(parameters[f"a{i + 1}"][2]), 4),
                    round_a_number(
                        100 * np.abs(float(parameters[f"a{i + 1}"][2]) / float(parameters[f"a{i + 1}"][0])), 3
                    ),
                )

            else:
                parameters_string += r"$\text{{{}}} = {}^{{+ {} ({}\%) }}_{{- {} ({}\%)}}$".format(
                    f"a{i + 1}",
                    round_a_number(parameters[f"a{i + 1}"][0], 6),
                    round_a_number(parameters[f"a{i + 1}"][1], 4),
                    round_a_number(
    ...
    if y_up is not None and y_down is not None:
        axes[0].set_title(
            rf"$\bfit{{Candidate\,\#{candidate_idx}}}$"
            + "\n$\\chi^2$/NDF = {}/{}, p-value = {}, RMSE = {}".format(
                func_candidate["Chi2"],
                func_candidate["NDF"],
                round_a_number(func_candidate["p-value"], 4),
                func_candidate["RMSE"],
            ),
            loc="right",
            size=9.5,
        ...
        axes[0].set_title(
            rf"$\bfit{{Candidate\,\#{candidate_idx}}}$"
            + "\nRMSE = {}, R2 = {}".format(func_candidate["RMSE"], func_candidate["R2"]),
            loc="right",
            size=9.5,
        )
    ...
    # Plot a panel showing the difference between the prediction and the input data point,
    # essentially (y_pred - y_label) in sigmas.
    axes[1].plot(x0, np.zeros(central.shape), c="black", linestyle="dashed", linewidth=linewidth, alpha=0.5)

    if y_up is not None and y_down is not None:
        residual_central = y - central_hist

        # Plot the residual error in terms of the input y uncertainties.
        y_unc_central = np.where(
            residual_central < 0, np.where(y_up != 0, y_up, y_down), np.where(y_down != 0, y_down, y_up)
        )
        ...
        if bin_widths_1d is not None:
            ...
            axes[1].scatter(x, residual_central / y_unc_central, marker=".", c="r", alpha=alpha)

        axes[1].set_ylim(
            -1.3 * max(np.abs(residual_central / y_unc_central)), 1.3 * max(np.abs(residual_central / y_unc_central))
        )
        ...
        # axes[1].plot(x0, -np.ones(central.shape).flatten(), c='grey', marker='none', alpha=0.7)
        # axes[1].fill_between(
        #     x0.flatten(), np.ones(central.shape).flatten(),
        #     -np.ones(central.shape).flatten(), color='grey', alpha=0.15, rasterized=True,
        # )
    else:
        if bin_widths_1d is not None:
            axes[1].bar(
            ...
            axes[1].scatter(x, y - central_hist, marker=".", c="r", alpha=alpha)

        axes[1].set_ylim(-1.3 * max(np.abs(y - central_hist)), 1.3 * max(np.abs(y - central_hist)))
    ...
    if param_shifted is not None:
        if func_candidate["Parameters: (best-fit, +1, -1)"][param_shifted][1] > 0:
            if y_up is not None and y_down is not None:
                ...
                np.where(residual_up < 0, np.where(y_up != 0, y_up, y_down), np.where(y_down != 0, y_down, y_up))

                residual_down = y - down_hist

                np.where(residual_down < 0, np.where(y_up != 0, y_up, y_down), np.where(y_down != 0, y_down, y_up))
    ...
    # Ratio plot: (function evaluated with one of the parameters shifted) /
    # (function evaluated with all parameters in best-fit).
    ...
    if param_shifted is not None:
        if func_candidate["Parameters: (best-fit, +1, -1)"][param_shifted][1] > 0:
            axes[2].plot(x0, up / central, color="g", linewidth=linewidth, alpha=alpha)

            axes[2].plot(x0, down / central, color="b", linewidth=linewidth, alpha=alpha)
    ...
    ax_inset = fig.add_axes([bbox.x0 + 0.73, bbox.y1 + 0.085, logo_width, logo_height])
...
def plot_all_syst_all_func_1D(func_candidates, x, bin_widths_1d, y, y_up, y_down, pdf_path, logy, logx):
    """
    ...
    with PdfPages(pdf_path) as pdf:
        for i in range(len(func_candidates)):
            ...
            axes.axis("off")
            plt.text(
                0.5, 0.5, f"Candidate function #{len(func_candidates) - 1 - i}", fontsize=20, ha="center", va="center"
            )
            ...
            if i < len(func_candidates) - 1:
                print(f"Plotting candidate functions {i + 1}/{len(func_candidates)} >>> {pdf_path}", end="\r")

            else:
                print(f"Plotting candidate functions {i + 1}/{len(func_candidates)} >>> {pdf_path}")
            ...
            if len(func_candidate["Parameters: (best-fit, +1, -1)"]) > 0:
                ...
                if has_uncert:
                    for j in range(len(func_candidate["Parameters: (best-fit, +1, -1)"])):
                        if func_candidate["Parameters: (best-fit, +1, -1)"][f"a{j + 1}"][1] > 0:
                            plot_single_syst_single_func_1D(
                                func_candidate=func_candidate,
...
def plot_total_unc_coverage_single_func_1D(
    func_candidate, candidate_idx, x, bin_widths_1d, y, y_up, y_down, n_samples, sampling_95quantile, logy, logx
):
    """
    ...
    if n_samples is not None:
        func_bands, func_bands_finer = func_sampling_1d(
            func_str=func_candidate["Parameterized equation, unscaled"],
        ...
        (mean_func_finer, lower_2sigma_finer, lower_1sigma_finer, upper_1sigma_finer, upper_2sigma_finer) = (
            func_bands_finer
        )
        ...
        axes[0].fill_between(
            x0, lower_1sigma_finer, upper_1sigma_finer, color="limegreen", alpha=0.5, label="68% quantile range"
        )

        if sampling_95quantile:
            axes[0].fill_between(x0, lower_2sigma_finer, lower_1sigma_finer, color="gold", alpha=0.5)

            axes[0].fill_between(
                x0, upper_1sigma_finer, upper_2sigma_finer, color="gold", alpha=0.5, label="95% quantile range"
            )
    ...
    if n_samples is not None:
        axes[0].set_title(
            rf"$\bfit{{Candidate\,\#{candidate_idx}}}$" + "\nEnsemble of functions generated by sampling parameters",
            loc="right",
            size=9.5,
        )
    else:
        axes[0].set_title(rf"$\bfit{{Candidate\,\#{candidate_idx}}}$" + "\n", loc="right", size=9.5)

    formatter = ticker.FormatStrFormatter("%.3g")

    # Plot a panel showing the difference between the prediction and the input data point,
    # essentially (y_pred - y_label) in sigmas.
    axes[1].plot(x0, np.zeros(x0.shape), c="black", linestyle="dashed", linewidth=linewidth, alpha=0.5)
    ...
    if y_up is not None and y_down is not None:
        ...
        # Plot the residual error in terms of the input y uncertainties.
        y_unc_mean = np.where(residual_mean < 0, np.where(y_up != 0, y_up, y_down), np.where(y_down != 0, y_down, y_up))

        if n_samples is not None:
            y_unc_upper_1sigma = np.where(
                residual_upper_1sigma < 0, np.where(y_up != 0, y_up, y_down), np.where(y_down != 0, y_down, y_up)
            )

            y_unc_lower_1sigma = np.where(
                residual_lower_1sigma < 0, np.where(y_up != 0, y_up, y_down), np.where(y_down != 0, y_down, y_up)
            )

            if sampling_95quantile:
                y_unc_upper_2sigma = np.where(
                    residual_upper_2sigma < 0, np.where(y_up != 0, y_up, y_down), np.where(y_down != 0, y_down, y_up)
                )

                y_unc_lower_2sigma = np.where(
                    residual_lower_2sigma < 0, np.where(y_up != 0, y_up, y_down), np.where(y_down != 0, y_down, y_up)
                )
        ...
        if bin_widths_1d is not None:
            if n_samples is not None:
                for i in range(x.size):
                    axes[1].hlines(
                        (residual_mean / y_unc_mean).flatten()[i],
                ...
                if sampling_95quantile:
                    axes[1].bar(
                        x.flatten(),
                        (
                            -residual_lower_1sigma / y_unc_lower_1sigma + residual_lower_2sigma / y_unc_lower_2sigma
                        ).flatten(),
                        width=bin_widths_1d.flatten(),
                    ...
                    axes[1].bar(
                        x.flatten(),
                        (
                            residual_upper_2sigma / y_unc_upper_2sigma - residual_upper_1sigma / y_unc_upper_1sigma
                        ).flatten(),
                        width=bin_widths_1d.flatten(),
            ...
            axes[1].scatter(x, residual_mean / y_unc_mean, marker=".", c="r", alpha=alpha)
        ...
        if n_samples is not None:
            if sampling_95quantile:
                axes[1].set_ylim(
                    ...
                    * np.max(
                        np.maximum(
                            np.abs(residual_lower_2sigma / y_unc_lower_2sigma),
                ...
                axes[1].set_ylim(
                    ...
                    * np.max(
                        np.maximum(
                            np.abs(residual_lower_1sigma / y_unc_lower_1sigma),
            ...
            axes[1].set_ylim(
                -1.3 * max(np.abs(residual_mean / y_unc_mean)), 1.3 * max(np.abs(residual_mean / y_unc_mean))
            )
        ...
        if bin_widths_1d is not None:
            ...
            axes[1].scatter(x, residual_mean, marker=".", c="r", alpha=alpha)

        axes[1].set_ylim(-1.3 * max(np.abs(residual_mean)), 1.3 * max(np.abs(residual_mean)))
    ...
    if n_samples is not None:
        axes[2].plot(x0, upper_1sigma_finer / mean_func_finer, color="limegreen", linewidth=linewidth, alpha=1)

        axes[2].plot(x0, lower_1sigma_finer / mean_func_finer, color="limegreen", linewidth=linewidth, alpha=1)

        if sampling_95quantile:
            axes[2].plot(x0, upper_2sigma_finer / mean_func_finer, color="gold", linewidth=linewidth, alpha=1)

            axes[2].plot(x0, lower_2sigma_finer / mean_func_finer, color="gold", linewidth=linewidth, alpha=1)
...
def plot_total_unc_coverage_all_func_1D(
    func_candidates, x, bin_widths_1d, y, y_up, y_down, n_samples, sampling_95quantile, pdf_path, logy, logx
):
    """
    ...
    with PdfPages(pdf_path) as pdf:
        for i in range(len(func_candidates)):
            ...
            if i < len(func_candidates) - 1:
                print(
                    f"Plotting candidate functions (sampling parameters) {i + 1}/{len(func_candidates)} >>> {pdf_path}",
                    end="\r",
                )
            ...
            if len(func_candidate["Parameters: (best-fit, +1, -1)"]) > 0:
                ...
                if has_uncert:
                    try:
                        plot_total_unc_coverage_single_func_1D(
                            func_candidate=func_candidate,
                            candidate_idx=len(func_candidates) - 1 - i,
                            x=x,
                            bin_widths_1d=bin_widths_1d,
...
def plot_single_syst_single_func_2D(
    ...
    fig, axes = plt.subplots(
        2, 2, sharex=True, sharey=True, figsize=(9, 7), gridspec_kw={"width_ratios": [1, 1], "height_ratios": [1, 1]}
    )
    # fig.subplots_adjust(hspace = 0.1)
    cbar_fontsize = 14
    label_fontsize = 12
    ...
    hist, x_edges, y_edges = np.histogram2d(
        np.array(x)[:, 0], np.array(x)[:, 1], bins=[x0_bins, x1_bins], weights=np.squeeze(y)
    )
    ...
    if contour is not None:
        ...
        axes[0, 0].contour(X[:-1, :-1], Y[:-1, :-1], hist_masked.T, levels=[contour], colors="red")
    ...
    central_smooth = func_evaluate(func_candidate=func_candidate, x=x_smooth, dim=2, param_shifted=None, sigma_pm=None)

    # Plot the candidate function (smooth)
    if logy:
        fig_fitted_smooth = axes[0, 1].hist2d(
            x_smooth[:, 0],
            x_smooth[:, 1],
    ...
    if contour is not None:
        axes[0, 1].contour(
            x0_smooth, x1_smooth, central_smooth.reshape(x0_nbins, x1_nbins), levels=[contour], colors="red"
        )
    ...
    central_hist = func_evaluate(func_candidate=func_candidate, x=x, dim=2, param_shifted=None, sigma_pm=None)

    central_error = y - central_hist

    if y_up is not None and y_down is not None:
        y_unc = np.where(central_error < 0, np.where(y_up != 0, y_up, y_down), np.where(y_down != 0, y_down, y_up))
    ...
    central_hist_full = func_evaluate(func_candidate=func_candidate, x=x_full, dim=2, param_shifted=None, sigma_pm=None)

    # Plot the candidate function in original bins
    if logy:
        fig_fitted_hist = axes[1, 0].hist2d(
            x_full[:, 0],
            x_full[:, 1],
            bins=[x0_bins, x1_bins],
    ...
    if contour is not None:
        axes[1, 0].contour(
            x0_mesh,
            x1_mesh,
            central_hist_full.reshape(x1_bins.size - 1, x0_bins.size - 1),
            levels=[contour],
            colors="red",
        )
    ...
    if y_up is not None and y_down is not None:
        cbar_error = plt.colorbar(
            fig_error[3], ax=axes[1, 1], pad=0, label="$\\frac{\\text{Data}-\\text{Fit}}{\\text{Data unc.}}$"
        )

    else:
        cbar_error = plt.colorbar(fig_error[3], ax=axes[1, 1], pad=0, label="$\\text{Data}-\\text{Fit}$")
...
def plot_all_syst_all_func_2D(
    func_candidates, x, bin_edges_2d, y, y_up, y_down, pdf_path, logx0, logx1, logy, cbar_min, cbar_max, cmap, contour
):
    """
    Plot all candidate functions, each with all possible parameter variations.
        ...
        for i in range(len(func_candidates)):
            ...
            if False:  # fix later
                ...
                if has_uncert:
                    for j in range(len(func_candidate["Parameters: (best-fit, +1, -1)"])):
                        if func_candidate["Parameters: (best-fit, +1, -1)"][f"a{j + 1}"][1] > 0:
                            plot_single_syst_single_func_2D(
                                func_candidate=func_candidate,
                                candidate_idx=len(func_candidates) - 1 - i,
                                x=x,
...
def plot_correlation(func_candidate, candidate_idx, y_up, y_down):
    """
    ...
    ax_inset = fig.add_axes([0.832, 0.955, logo_width, logo_height])
...
def plot_all_corr(func_candidates, y_up, y_down, pdf_path):
    """
    ...
    with PdfPages(pdf_path) as pdf:
        for i in range(len(func_candidates)):
            ...
            if i < len(func_candidates) - 1:
                print(f"Plotting correlation matrices {i + 1}/{len(func_candidates)} >>> {pdf_path}", end="\r")

            else:
                print(f"Plotting correlation matrices {i + 1}/{len(func_candidates)} >>> {pdf_path}")
...
def plot_gof(func_candidates, gof, log_scale=True):
    """
    ...
    ax_inset = fig.add_axes([0.85, 0.945, logo_width, logo_height])
...
def plot_all_gof(func_candidates, y_up, y_down, pdf_path):
    """
    ...
    print(f"Plotting goodness-of-fit scores >>> {pdf_path}")
    ...
    with PdfPages(pdf_path) as pdf:
        for i in range(len(gof)):
            plot_gof(func_candidates=func_candidates, gof=gof[i], log_scale=True)
