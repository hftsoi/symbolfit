...
def plot_single_syst_single_func_1D(func_candidate, candidate_idx, x, bin_widths_1d, y, y_up, y_down, param_shifted, logy, logx):
    '''
    ...
    central = func_evaluate(func_candidate, x0, 1)
    ...
    if param_shifted is not None:
        if func_candidate['Parameters: (best-fit, +1, -1)'][param_shifted][1] > 0:
            up = func_evaluate(func_candidate, x0, 1, param_shifted=param_shifted, sigma_pm='+')
            down = func_evaluate(func_candidate, x0, 1, param_shifted=param_shifted, sigma_pm='-')
    ...
    central_hist = func_evaluate(func_candidate, x, 1)
    # Function evaluated with one of the parameters shifted up and down separately (with the same input x grid).
    if param_shifted is not None:
        if func_candidate['Parameters: (best-fit, +1, -1)'][param_shifted][1] > 0:
            up_hist = func_evaluate(func_candidate, x, 1, param_shifted=param_shifted, sigma_pm='+')
            down_hist = func_evaluate(func_candidate, x, 1, param_shifted=param_shifted, sigma_pm='-')
    ...
    if y_up is not None and y_down is not None:
        if bin_widths_1d is not None:
            axes[0].errorbar(x.flatten(), y.flatten(), yerr=[y_down.flatten(), y_up.flatten()], xerr=bin_widths_1d.flatten()/2, fmt='.', c='black', ecolor='grey', capsize=0, label='Data')
        else:
            axes[0].errorbar(x.flatten(), y.flatten(), yerr=[y_down.flatten(), y_up.flatten()], fmt='.', c='black', ecolor='grey', capsize=0, label='Data')
    else:
    ...
    if param_shifted is not None:
        if func_candidate['Parameters: (best-fit, +1, -1)'][param_shifted][1] > 0:
            axes[0].plot(x0, up, color='g', label='{0} Up ($+1\\sigma$)'.format(param_shifted), linewidth=linewidth, alpha=alpha, linestyle='dashed')
            axes[0].plot(x0, down, color='b', label='{0} Down ($-1\\sigma$)'.format(param_shifted), linewidth=linewidth, alpha=alpha, linestyle='dashed')
    axes[0].legend()
    ...
    if y_up is not None and y_down is not None:
        ...
        if bin_widths_1d is not None:
            axes[1].bar(x.flatten(), (residual_central/y_unc_central).flatten(), width=bin_widths_1d.flatten(), edgecolor='none', color='red', alpha=alpha)
        else:
            axes[1].scatter(x, residual_central/y_unc_central, marker='.', c='r', alpha=alpha)
        
        axes[1].set_ylim(-1.3*max(np.abs(residual_central/y_unc_central)), 1.3*max(np.abs(residual_central/y_unc_central)))
        ...
        if bin_widths_1d is not None:
            axes[1].bar(x.flatten(), (y - central_hist).flatten(), width=bin_widths_1d.flatten(), edgecolor='none', color='red', alpha=alpha)
        else:
...
def plot_all_syst_all_func_1D(func_candidates, x, bin_widths_1d, y, y_up, y_down, pdf_path, logy, logx):
    ...
    with PdfPages(pdf_path) as pdf:
        for i in range(len(func_candidates)):
            ...
            axes.axis('off')
            plt.text(0.5, 0.5, 'Candidate function #{}'.format(len(func_candidates) - 1 - i), fontsize = 20, ha = 'center', va = 'center')
            plt.tight_layout()
            ...
            if len(func_candidate['Parameters: (best-fit, +1, -1)']) > 0:
                ...
                if has_uncert:
                    for j in range(len(func_candidate['Parameters: (best-fit, +1, -1)'])):
                        if func_candidate['Parameters: (best-fit, +1, -1)'][f'a{j+1}'][1] > 0:
                            plot_single_syst_single_func_1D(func_candidate, len(func_candidates) - 1 - i, x, bin_widths_1d, y, y_up, y_down, f'a{j+1}', logy, logx)
                            plt.savefig(pdf, format='pdf')
                            plt.close()
                else:
                    plot_single_syst_single_func_1D(func_candidate, len(func_candidates) - 1 - i, x, bin_widths_1d, y, y_up, y_down, None, logy, logx)
                    plt.savefig(pdf, format='pdf')
                    plt.close()
            else:
                plot_single_syst_single_func_1D(func_candidate, len(func_candidates) - 1 - i, x, bin_widths_1d, y, y_up, y_down, None, logy, logx)
...
def plot_single_syst_single_func_2D(func_candidate, candidate_idx, x, bin_edges_2d, y, y_up, y_down, param_shifted, logy, logx):
    '''
    Plot a particular candidate function with all parameters at their best-fit values,
    or with one of the parameters in +1 or -1 sigma.
    ...
    fig, axes = plt.subplots(2, 2, sharex = True, sharey = True, figsize = (9, 7), gridspec_kw = {'width_ratios': [1, 1], 'height_ratios': [1, 1]})
    #fig.subplots_adjust(hspace = 0.1)
    alpha = 1
    cbar_fontsize = 14
    label_fontsize = 12
    ...
    # Plot input
    fig_data = axes[0,0].hist2d(x[:,0], x[:,1], bins=[x0_bins, x1_bins], weights=np.squeeze(y), cmap='Greens', edgecolor='none', rasterized=True)
    ...
    central_smooth = func_evaluate(func_candidate, x_smooth, 2)
    
    # Plot the candidate function (smooth)
    fig_fitted_smooth = axes[0,1].hist2d(x_smooth[:,0], x_smooth[:,1], bins=(x0_nbins, x1_nbins), weights=np.squeeze(central_smooth), cmap='Greens', edgecolor='none', rasterized=True)
    ...
    central_hist = func_evaluate(func_candidate, x, 2)
    ...
    fig_fitted_hist = axes[1,0].hist2d(x[:,0], x[:,1], bins=[x0_bins, x1_bins], weights=np.squeeze(central_hist), cmap='Greens', edgecolor='none', rasterized=True)
    ...
    fig_error = axes[1,1].hist2d(x[:,0], x[:,1], bins=[x0_bins, x1_bins], weights=np.squeeze(central_error), cmap='bwr', vmin=-max(np.abs(central_error)), vmax=max(np.abs(central_error)), edgecolor='none', rasterized=True)
    if y_up is not None and y_down is not None:
        cbar_error = plt.colorbar(fig_error[3], ax=axes[1,1], pad=0, label='$\\frac{\\text{Data}-\\text{Fit}}{\\text{Uncertainty}}$ $(\\sigma)$')
    else:
...
def plot_all_syst_all_func_2D(func_candidates, x, bin_edges_2d, y, y_up, y_down, pdf_path, logy, logx):
    '''
    Plot all candidate functions, each with all possible parameter variations.
        ...
        for i in range(len(func_candidates)):
            ...
            if False:
                ...
                if has_uncert:
                    for j in range(len(func_candidate['Parameters: (best-fit, +1, -1)'])):
                        if func_candidate['Parameters: (best-fit, +1, -1)'][f'a{j+1}'][1] > 0:
                            plot_single_syst_single_func_2D(func_candidate, len(func_candidates) - 1 - i, x, bin_edges_2d, y, y_up, y_down, f'a{j+1}', logy, logx)
                            plt.savefig(pdf, format='pdf')
                            plt.close()
                else:
                    plot_single_syst_single_func_2D(func_candidate, len(func_candidates) - 1 - i, x, bin_edges_2d, y, y_up, y_down, None, logy, logx)
                    plt.savefig(pdf, format='pdf')
                    plt.close()
            else:
                plot_single_syst_single_func_2D(func_candidate, len(func_candidates) - 1 - i, x, bin_edges_2d, y, y_up, y_down, None, logy, logx)
...
def plot_all_corr(func_candidates, y_up, y_down, pdf_path):
    ...
    Returns
    -------
    Plot all correlation matrices and write to an output file.
    '''
...
def plot_gof(func_candidates, gof, log_scale = True):
    ...
    for i, idx in enumerate(candidate_idx):
        plt.annotate(idx, (func_candidates['Complexity'][i], func_candidates[gof][i]), textcoords = "offset points",
                     xytext = (0, 5), ha = 'center', size = 13)
