...
def round_numbers_in_sympy_expr(sympy_expr, sig_fig=3):
    ...
    rounded_expr = sympy_expr.replace(lambda x: x.is_Number, lambda x: rounding(x, sig_fig))
...
def parse_pysr_equ(pysr_dir, x):
    ...
    if model.expression_spec is None:
        ...
        model.equations_['equation'][i]:
        f = (#1 * 0.7463786) + 4.54765;
        g = ((#1 * ((#2 + -0.8933617) * (#1 + -1.0033985))) * -4.229166) + 0.6955875;
        d = [0.04280354, -0.62839013, 7.853348]
        """
        ...
        df = pd.DataFrame(
            {
                "PySR template spec": equation_spec,
                "PySR equation": equation,
